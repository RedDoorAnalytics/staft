...
program staft_pred
	...
	// Linear predictor or AF
		...
		if "`af'"!="" {
			predictnl double `newvarname' = exp(-xb(xb)) if `touse', `ciopt'
		}
	...
	// afnum and afdenom
		...
		if "`afnum'"!="" {
			...
			if "`afdenom'"=="" {
				predictnl double `newvarname' = predict(af at(`afnum'))/predict(af zeros) if `touse', `ciopt'
			}	
			else {
				predictnl double `newvarname' = predict(af at(`afnum'))/predict(af at(`afdenom')) if `touse', `ciopt'
			}
