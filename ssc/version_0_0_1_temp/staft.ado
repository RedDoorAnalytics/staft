...
/*
To do
-> post final knots when varying
-> post estimation, inc. help file
...
-> make it fit stpm2 when no covariates specified
-> check set obs missing problem
-> need markout on tvc's
*/

program staft, eclass sortpreserve properties(st)
	version 12.1
	if "`0'"=="" {
		if (`"`e(cmd)'"'!="staft") error 301
		Replay `0'
	}
	else {
		Estimate `0'
...
program Estimate, eclass
	...
	// Starting values 
		...
		if !`stpm2' {
			...
			if "`fixedknots'"=="" {
				...
				//don't need to fit stpm2 model, just use it to calculate tvc's
				if "`tvc'"!=""{
					qui stpm2, scale(h) `df' `knots' `noorthog' level(`level') `bknots' `bknotstvc' tvc(`tvc') `dftvc' `knotstvc' `knscale' iter(0)
				}
			
				//get xb for starting knots
				tempvar tempxb
				quietly predict double `tempxb' if `touse', xb
				quietly replace `tempxb' = `tempxb'- [xb][_cons] if `touse'
				
				tempvar templogtxb coxindex S Sadj Z
				quietly gen double `templogtxb' = log(_t* exp(-`tempxb')) if `touse'
				
				if "`noorthog'"=="" local orthopt orthog
				if "`df'"!="" local if2 if2(_d==1)
				qui rcsgen `templogtxb' if `touse', gen(_temp_rcs) `df' `knots' `bknots' `orthopt' `if2'
				
				local temprcsterms `r(rcslist)'
				local Nsplines : word count `temprcsterms'
				local ln_bknots `r(knots)'
				...
				qui stcox `varlist' if `touse', estimate //tvc(`varlist') texp(ln(_t))
				...
				qui regress `Z' `temprcsterms'  if `touse' & _d == 1 , `constant'
				matrix `initmat2' = e(b)
				mat `initmat1' = `initmat1',`initmat2'
	...
	// Maximisation
		...
		if "`lininit'"!="" {
			quietly staft `varlist', df(1) `fixedknots'
			mat `initmat1' = e(b)
		...
		if "`fixedinit'"!="" {
			staft `varlist', fixedknots `df' `knots' `noorthog' level(`level') `bknots' `bknotstvc' tvc(`tvc') `dftvc' `knotstvc' `knscale'
			mat `initmat1' = e(b)
			local initopt init(`initmat1',copy)
			
			tempvar tempxb2
			quietly predict double `tempxb2' if `touse', xb
				
			quietly replace `templogtxb' = log(_t* exp(-`tempxb2')) if `touse'
		...
		ml model `mlmethod'`debug' staft_lf2() `mleqns'						///
										if `touse'						///
										`wt',							///
										init(`initmat1',copy)			///
										`options' 						///
